...
|---|---|
| `data/personnages.csv` | **Source de vérité** : 173 lignes × 16 colonnes, UTF-8 `;`. C'est le seul fichier à éditer à la main. |
...
| `base_personnages_fictifs.xlsx` | Classeur **public** : *Personnages* (173 × 16) + *Lisez-moi*. **Sans** la narration. |
...
| `tests/test_base.py` | **38 garde-fous** couvrant toutes les conventions, lancés en CI. |
...
# 4. contrôler la base
python3 -m unittest discover -s tests -v   # 38 garde-fous
ruff check .                               # lint
```
...
- Les **38 garde-fous** vérifient notamment : effectifs et cohérence des 4
